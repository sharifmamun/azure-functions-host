        ...
        : IClassFixture<TTestFixture> where TTestFixture : ApplicationInsightsTestFixture, new()
    {
        ...
        private string ValidateEndToEndTest(string functionName, string functionTrace, bool functionSuccess)
        {
            ...
            TraceTelemetry trace = _fixture.Channel.Telemetries
                    ...
                    .Where(t => t.Message.Contains(functionTrace) && t.Properties[LogConstants.CategoryNameKey] == LogCategories.CreateFunctionUserCategory(functionName))
                    .Single();
        ...
        public async Task Validate_HostLogs()
        {
            ...
            traces = traces.Where(t => 
                ...
                t.Properties[LogConstants.CategoryNameKey] != LanguageWorkerConstants.FunctionConsoleLogCategoryName &&
            ...
            ValidateTrace(traces[idx++], "Adding Function descriptor providers for all languages.", LogCategories.Startup);
            ValidateTrace(traces[idx++], "Creating function descriptors.", LogCategories.Startup);
            ValidateTrace(traces[idx++], "Debug file watch initialized.", LogCategories.Startup);
            ValidateTrace(traces[idx++], "Diagnostic file watch initialized.", LogCategories.Startup);
            ValidateTrace(traces[idx++], "File event source initialized.", LogCategories.Startup);
            ValidateTrace(traces[idx++], "Found the following functions:\r\n", LogCategories.Startup);
            ValidateTrace(traces[idx++], "Found worker config:", "Host.LanguageWorkerConfig");
            ValidateTrace(traces[idx++], "Found worker config:", "Host.LanguageWorkerConfig");
            ValidateTrace(traces[idx++], "Function descriptors created.", LogCategories.Startup);
            ...
            ValidateTrace(traces[idx++], "Will load worker provider for language: java", "Host.LanguageWorkerConfig");
            ValidateTrace(traces[idx++], "Will load worker provider for language: node", "Host.LanguageWorkerConfig");
            ValidateTrace(traces[idx++], "Worker path for language worker java:", "Host.LanguageWorkerConfig");
            ValidateTrace(traces[idx++], "Worker path for language worker node:", "Host.LanguageWorkerConfig");
            ValidateTrace(traces[idx++], "Workers Directory set to:", "Host.LanguageWorkerConfig");
        }
